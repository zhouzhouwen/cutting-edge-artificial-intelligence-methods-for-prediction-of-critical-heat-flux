...
feature_columns = ['Tube Diameter', 'Heated Length', 'Pressure', 'Mass Flux', 'Outlet Quality', 'Inlet Temperature', 'latent_heat_of_vaporization', 'saturated_liquid_enthalpy']
...
def save_trial_callback(study, trial):
    ...
    # 将每次试验的记录追加到CSV文件中
    pd.DataFrame([trial_record]).to_csv("Transformer_optuna_trials_1_eight_input.csv", mode='a', header=False, index=False)

# 在开始优化之前重置CSV文件
columns = ["number", "value", "params", "datetime_start", "datetime_complete"]
pd.DataFrame(columns=columns).to_csv("Transformer_optuna_trials_1_eight_input.csv", index=False)
