...
for column in columns:
    all_input[column] = scaler_standard[column].fit_transform(all_input[[column]])
# print(all_input)
...
all_input_pre = all_input_pre.reshape(-1, 1)

all_input_pre = scaler_standard[last_column_name].inverse_transform(all_input_pre)
print(all_input_pre)
all_input_pre_df = pd.DataFrame(all_input_pre)

# Concatenate all dataframes horizontally
final_df = pd.concat([data_to_transform[columns], all_input_pre_df], axis=1)

# Save the combined dataframe to a CSV file
all_input_pre_df.to_csv('Final_Dataset.csv', index=False)
