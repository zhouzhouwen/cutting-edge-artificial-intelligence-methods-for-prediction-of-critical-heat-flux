...
def save_trial_callback(study, trial):
    ...
    # 将每次试验的记录追加到CSV文件中
    pd.DataFrame([trial_record]).to_csv("Transformer_optuna_trials_3.csv", mode='a', header=False, index=False)

# 在开始优化之前重置CSV文件
columns = ["number", "value", "params", "datetime_start", "datetime_complete"]
pd.DataFrame(columns=columns).to_csv("Transformer_optuna_trials_3.csv", index=False)
