...
slice_file_path = '/home/user/ZHOU-Wen/phase1-chf-exercises-main/data/inputs/Slice_01.csv'
lut_file_path = '/home/user/ZHOU-Wen/phase1-chf-exercises-main/data/results/LUT/Slice_01_LUT.csv'
sheet_name = 'slice01'
...
# 删除第二列
numeric_data = np.delete(numeric_data, 1, axis=1)

# Modifying the 'Pressure' column by dividing by 1e3
numeric_data[:, 2-1] /= 1e3
print(numeric_data)
...
data_to_transform = data_to_transform.drop(['Heated Length', 'Inlet Subcooling', 'Inlet Temperature', 'CHF'], axis=1)
...
slice_real_data_values = slice_real_data[['Tube Diameter', 'Pressure', 'Mass Flux', 'Outlet Quality',]].values
for i, column_name in enumerate(scalers):
    # print(column_name)
...
# 加载保存的模型权重
model.load_state_dict(torch.load('/home/user/ZHOU-Wen/phase1-chf-exercises-main/Zhou_Wen_Code/model/Transformer_best_model_1_four.pth'))
model.eval()  # 设置为评估模式
...
plt.scatter(slice_real_data['Tube Diameter'], slice_real_data['CHF'], color='purple', marker='D',  label='Real data', zorder=2)
