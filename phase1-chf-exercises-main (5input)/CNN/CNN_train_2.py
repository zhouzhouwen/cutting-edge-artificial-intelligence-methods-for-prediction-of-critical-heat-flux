...
def GAF_transform(data):
    ...
    # 画图和颜色条
    # Increasing the figure size for better visibility and space for colorbar
    # fig, ax = plt.subplots(1, 2, figsize=(18, 6))
    #
    # # Gramian Angular Field
    # gaf_img = ax[0].imshow(data[0], cmap='rainbow', origin='lower')
    # ax[0].set_title("GAF", fontsize=18)  # Increase title font size
    # ax[0].tick_params(axis='both', which='major', labelsize=14)  # Increase font size for ticks
    # cbar_gaf = fig.colorbar(gaf_img, ax=ax[0], fraction=0.046, pad=0.04)
    # cbar_gaf.ax.tick_params(labelsize=14)  # Increase colorbar tick font size
    #
    #
...
def MTF_transform(data):
    ...
    # ax[1].set_title("MTF", fontsize=18)  # Increase title font size
    # ax[1].tick_params(axis='both', which='major', labelsize=14)  # Increase font size for ticks
    # cbar_mtf = fig.colorbar(mtf_img, ax=ax[1], fraction=0.046, pad=0.04)
    # cbar_mtf.ax.tick_params(labelsize=14)  # Increase colorbar tick font size
    #
    # # #
...
def save_trial_callback(study, trial):
    ...
    # 将每次试验的记录追加到CSV文件中
    pd.DataFrame([trial_record]).to_csv("CNN_optuna_trials_2.csv", mode='a', header=False, index=False)

# 在开始优化之前重置CSV文件
columns = ["number", "value", "params", "datetime_start", "datetime_complete"]
pd.DataFrame(columns=columns).to_csv("CNN_optuna_trials_2.csv", index=False)
