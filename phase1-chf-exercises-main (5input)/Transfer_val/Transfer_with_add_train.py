...
new_model_path = '/home/user/ZHOU-Wen/phase1-chf-exercises-main/Zhou_Wen_Code/Transfer_val/Transfer_model/Transfer_with_add_train_tube.pth'
...
class AddTransformerModel(nn.Module):
    def __init__(self, original_model, d_model):
        super(AddTransformerModel, self).__init__()
        # 复制原有模型的所有层
        ...
        # 添加额外的全连接层
        self.additional_linear1 = nn.Linear(1, d_model)
        self.additional_linear2 = nn.Linear(d_model, d_model)
        self.additional_linear3 = nn.Linear(d_model, 1)

    def forward(self, src):
        # 通过原有模型
        output = self.original_model(src)

        # 通过额外的全连接层
        output = self.additional_linear1(output)
        output = F.softsign(output)
        output = self.additional_linear2(output)
        output = F.softsign(output)
        output = self.additional_linear3(output)
        return output

# 实例化迁移学习模型
add_model = AddTransformerModel(model, d_model=d_model)

# 定义您的模型、优化器和损失函数
optimizer = torch.optim.Adam(add_model.parameters(), lr=0.001, weight_decay=1e-3)
# criterion = torch.nn.SmoothL1Loss()
criterion = torch.nn.L1Loss()

train_losses, val_losses, best_val_loss = train_and_evaluate(add_model, train_loader, val_loader, optimizer, criterion, epochs=2000)


# ————————————————————————————————————————————————————————————得到结果————————————————————————————————————————————————————————————

# 首先创建原始的 TransformerModel 实例
original_model = TransformerModel(input_features=len(input_columns), d_model=d_model, nhead=nhead,num_encoder_layers=num_encoder_layers, dim_feedforward=dim_feedforward, dropout=dropout)
# 然后使用原始模型创建 TransferTransformerModel 实例
add_model = AddTransformerModel(original_model, d_model=d_model)
# 加载保存的模型权重
add_model.load_state_dict(torch.load(new_model_path))
add_model.eval()  # 设置为评估模式
...
with torch.no_grad():
    y_train_pre = add_model(normalized_data_X_train)
    y_val_pre = add_model(normalized_data_X_val)
    y_test_pre = add_model(normalized_data_X_test)
